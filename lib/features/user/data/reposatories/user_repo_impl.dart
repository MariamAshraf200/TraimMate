...
class UserRepoImpl implements UserRepo{
  ...
  Future<Either<Failures, UserEntity>> login(String email, String password) async {
    if(await networkinfo.isConnected){
      try {
        ...
        UserEntity userEntity = UserEntity(id: usermodel.id,
            name: usermodel.name,
            email: email,
        ...
        throw WrongDataFailuer();
      }
    }else{
      throw OfflineFailuer();
    }
  }
  ...
  @override
  Future<Either<Failures, UserEntity>> signup(String email, String name, String phone, String password) async {
    if(await networkinfo.isConnected){
      try {
        final usermodel = await remoteDataScorce.signup(email, phone, name, password);
